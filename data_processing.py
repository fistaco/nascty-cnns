...
ASCAD_DIRECTORY = "./../ASCAD_data/ASCAD_databases"
CHIPWHISPERER_DIRECTORY = "./../Chipwhisperer"


def load_data(dataset_name, hw=False, noise_std=0.0, desync=0):
    """
    Loads the dataset with the given `dataset_name` according to the given
    countermeasure parameters and leakage type.
    """
    load_funcs = {
    ...
    x = list(load_funcs[dataset_name](hw=hw))
...
def load_ascad_data(data_filepath="./../ASCAD_data/ASCAD_databases/ASCAD.h5",
    ...
    # This should resolve to a path like ".../ASCAD_databases/set_name.h5"
    data_filepath = f"{ASCAD_DIRECTORY}/{set_name}.h5"
...
def load_prepared_ascad_vars(subkey_idx=2, scale=True, use_mlp=True,
                             for_sgd=False, hw=False, n_train=50000, desync=0):
    """
    ...
    (x_train, y_train, x_atk, y_atk, train_meta, atk_meta) = \
        load_ascad_data(load_metadata=True, desync=desync)
    original_input_shape = (700, 1)
...
def construct_countermeasure_datasets(dataset_name, desync=50):
    ...
    (x_train, _, _, _, x_atk, _, _, _) = \
        load_data(dataset_name, hw=False)
